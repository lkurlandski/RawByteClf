...
TASKS = ["clm", "mlm"]
SEEDS = [42, 0, 1, 2, 3]
TR_SAMPLES_PER_CLASSES = [1, 2, 3, 4, 5]
HOURS = [1, 2, 2, 2, 3]
...
CLF_GRADIENT_CHECKPOINTING = "false"
CLF_STREAMING = "false"
...
--tf32={TF32} \\
--gradient_checkpointing={CLF_GRADIENT_CHECKPOINTING}
"""
...
for algorithm in ALGORITHMS:
    for vocab_size in VOCAB_SIZES:
        ...
        for mode, task in zip(MODES, TASKS):
            ...
            #         job_name = f"ft-{'clf'}-{algorithm[0]}-{vocab_size}-{top_k}-{seed}"
            #         body = BODY_CLF \
            #             .replace("JOB_NAME", job_name) \
            #             .replace("ALGORITHM", algorithm) \
            #             .replace("VOCAB_SIZE", str(vocab_size)) \
            ...
            for tr_samples_per_class, hours in zip(TR_SAMPLES_PER_CLASSES, HOURS):
                for seed in SEEDS:
                    # job_name = f"ft-{'clf'}-{mode}-{algorithm[0]}-{vocab_size}-{top_k}-{seed}"
                    job_name = f"ft-{'clf-ksc'}-{algorithm[0]}-{vocab_size}-{tr_samples_per_class}-{seed}"
                    body = BODY_CLF \
                        .replace("JOB_NAME", job_name) \
                        .replace("ALGORITHM", algorithm) \
                        .replace("VOCAB_SIZE", str(vocab_size)) \
                        .replace("TR_SAMPLES_PER_CLASS", str(tr_samples_per_class)) \
                        .replace("SEED", str(seed)) \
                    ...
                    for f in checkpoints:
                        job_name = f"ft-{'clf-ksc-ft-' + f.name.replace('checkpoint-', '')}-{algorithm[0]}-{vocab_size}-{tr_samples_per_class}-{seed}"
                        body = BODY_CLF \
                            .replace("JOB_NAME", job_name) \
                            .replace("ALGORITHM", algorithm) \
                            .replace("VOCAB_SIZE", str(vocab_size)) \
