...
def create_the_file_of_shas():
    ...
    materials = get_materials_esp_det(ll)
    shas = list((af.name for af in chain.from_iterable(materials.files.values())))
    allshas.extend(shas)

    materials = get_materials_esp_fam(ll)
    shas = list((af.name for af in chain.from_iterable(materials.files.values())))
    allshas.extend(shas)

    materials = get_materials_esp_beh(ll)
    shas = list((af.name for af in chain.from_iterable(materials.files.values())))
    allshas.extend(shas)

    allshas = sorted(set(allshas))
    sha_file = Path("./tmp/shas_to_turn_into_ember_features.txt")
    sha_file.write_text("\n".join(allshas))


def process_archive(a, shas, extractor):
    print(str(os.getpid()), "processing", str(len(shas)), "shas from", str(a))

    if not shas:
        return []

    stem = os.path.basename(a).split(".")[0]
    if not any(s[0:len(stem)] == stem for s in shas):
        return []
 
    index = []
    data  = []
    with ZipFile(a) as zp:
        for n in zp.namelist():
            ...
            b = zp.read(n)
            try:
                x = extractor.feature_vector(b)
            except Exception as err:
                print("A fatal error occurred trying to process: ", s)
                continue
            index.append(s)
            data.append(x) 

    return list(zip(index, data))


def main():

    sha_file = Path("./tmp/shas_to_turn_into_ember_features.txt")
    shas = set(s.split(".")[0] for s in sha_file.read_text().split("\n"))
    print("Found shas to work with: ", len(shas))

    archives = sorted(glob("./data/*/nop/*.zip", recursive=True))
    print("Found archives to work with: ", len(archives))

    extractor = PEFeatureExtractor()
    print("Using the extractor: ", extractor)


    iterable = []
    for a in archives:
        stem = os.path.basename(a).split(".")[0]
        _shas = set(s for s in shas if s[0:len(stem)] == stem)
        iterable.append((a, _shas, extractor))

    print("Mutliprocessing on items:", str(len(iterable)))
    with mp.Pool(16) as pool:
        output = pool.starmap(process_archive, iterable)
    print("Mutliprocessing finished. Recieved:", str(len(output)))

    index = []
    data  = []
    for o in output:
        index.extend([i for i, d in o])
        data.extend([d for i, d in o])

    np.save("./cache/ember/features.npy", np.stack(data))
    Path("./cache/ember/index.txt").write_text("\n".join(index))
    ...
    data  = []
    i = 0
    pbar = tqdm(archives, total=len(shas))
    for a in pbar:
        if not shas:
            break
        stem = os.path.basename(a).split(".")[0]
        ...
        with ZipFile(a) as zp:
            for n in zp.namelist():
                ...
                if s not in shas:
                    continue
                i += 1
                pbar.set_description(str(i) + "/" + str(pbar.total) + ": " + s)
                shas.remove(s)
                b = zp.read(n)
                try:
                    x = extractor.feature_vector(b)
